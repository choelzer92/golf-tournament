...
| **Review + merge `audit-sharing-login-2026-09-14`** | The audit + ALL THIRTEEN fixes (F-047…F-059) live there; nothing pushed. Cookie now: full 30d sliding, pool 48h (F-051+F-057 both in) |
| **Set `NEXT_PUBLIC_OWNER_GHIN`** (his real GHIN) in `.env.local` + the deploy env | Activates F-059: until set, the invite code still equals owner (legacy, deliberate rollout safety). One env var, then members are scoped to their own games |
