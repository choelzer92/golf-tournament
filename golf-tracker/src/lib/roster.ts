...
export async function hydrateRoster(): Promise<RosterPlayer[]> {
  const { data } = await supabase.from('players').select('id, ghin_number, name, handicap_index, gender');
  if (data) {
    rosterCache.clear();
...
export async function upsertRosterPlayer(player: RosterPlayer): Promise<RosterPlayer> {
  const existing = player.ghinNumber != null ? getRosterPlayerByGhin(player.ghinNumber) : null;
  const merged: RosterPlayer = { ...player, id: existing?.id || player.id };
