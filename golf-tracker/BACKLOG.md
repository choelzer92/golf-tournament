...
| **My-groups page shows no players** — "when i look at my group on the my groups page, players dont show up. Lets investigate this later." NOT yet diagnosed; seen on the LIVE app, so it's real data (his actual group), not a sandbox-seed artifact. Suspects worth checking first: viewer scoping in `hydrateGroups`/`getGroups` (ownerGhin mismatch), or group `playerIds` referencing roster rows the viewer can't see. | ? | Craig 2026-09-10 |
