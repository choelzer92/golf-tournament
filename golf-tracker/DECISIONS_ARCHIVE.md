# Decisions archive — the complete append-only log

This is the FULL historical record of Craig's decisions, corrections, and preferences,
with the reasoning. `DECISIONS.md` is the working set: the active constraints in full
plus a one-line index of everything here. **Append new decisions HERE in full** (same
format as always), and add the index line — plus the full text if it's an active
constraint — to `DECISIONS.md`.

Read this file section-by-section when a task touches a settled topic (grep for the
§ number from the index); don't read it whole at session start.
